...
public class ByteTree {
    ...
    private void add(byte[] bytes, String value) {
        byte curr_b = bytes[0];
    ...
    public String get(ByteBuf buf, int absPos, int length) {
        if (buf.hasArray()) {
    ...
    public String get(long address, int length) {
        Node node = root;
    ...
    static final class ArrayNode extends Node {
        ...
        ArrayNode(byte old_b, Node old_n, byte new_b, Node new_n, String value) {
